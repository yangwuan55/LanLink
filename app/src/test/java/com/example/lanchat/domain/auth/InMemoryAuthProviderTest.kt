...
class InMemoryAuthProviderTest {
    ...
    fun `InMemoryAuthProvider returns Success with correct 6-digit PIN`() = runBlocking {
        val provider = com.ymr.lancomm.data.auth.InMemoryAuthProvider("123456")
    ...
    fun `InMemoryAuthProvider returns Failure with wrong PIN`() = runBlocking {
        val provider = com.ymr.lancomm.data.auth.InMemoryAuthProvider("654321")
    ...
    fun `InMemoryAuthProvider default PIN is 6 digits`() {
        val provider = com.ymr.lancomm.data.auth.InMemoryAuthProvider()
    ...
    fun `PIN must be exactly 6 digits`() {
        assertThrows(IllegalArgumentException::class.java) {
            com.ymr.lancomm.data.auth.InMemoryAuthProvider("12345")  // Too short
        }
        assertThrows(IllegalArgumentException::class.java) {
            com.ymr.lancomm.data.auth.InMemoryAuthProvider("1234567")  // Too long
        }
    }

    @Test
    fun `getCredentials returns correct PIN bytes`() {
        val provider = com.ymr.lancomm.data.auth.InMemoryAuthProvider("999888")
