...
import com.ymr.lancomm.net.AuthHandshakeResult
...
class TcpSocketClient(
        ...
        while (currentRetry < maxRetries && _connectionState.value is ConnectionState.Connecting) {
            try {
                ...
                val connectedSocket = Socket().apply {
                    connect(java.net.InetSocketAddress(peer.host, peer.port), connectionTimeoutMs.toInt())
    ...
    override suspend fun authenticate(deviceName: String, credentials: ByteArray): AuthHandshakeResult =
        withContext(Dispatchers.IO) {
            ...
            val request = AuthRequest.newBuilder()
                ...
                .build()
            channel.sendAuthRequest(request)
            val response = channel.receiveAuthResponse()
            Log.d(TAG, "Auth response: success=${response.success}")
    ...
    override suspend fun send(type: Int, message: ByteArray): Unit = withContext(Dispatchers.IO) {
        val channel = _protobufChannel ?: throw IllegalStateException("Not connected")
        try {
