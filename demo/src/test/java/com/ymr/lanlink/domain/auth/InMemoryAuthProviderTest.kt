...
class InMemoryAuthProviderTest {
    ...
    fun `InMemoryAuthProvider returns Success with correct 6-digit PIN`() = runBlocking {
        val provider = com.ymr.lanlink.core.data.auth.InMemoryAuthProvider("123456")
        val credentials = "123456".toByteArray()
        val result = provider.authenticate("TestPeer", credentials)
        assertTrue(result is com.ymr.lanlink.core.domain.auth.AuthResult.Success)
    }

    @Test
    fun `InMemoryAuthProvider returns Failure with wrong PIN`() = runBlocking {
        val provider = com.ymr.lanlink.core.data.auth.InMemoryAuthProvider("654321")
        val credentials = "wrong".toByteArray()
        val result = provider.authenticate("TestPeer", credentials)
        assertTrue(result is com.ymr.lanlink.core.domain.auth.AuthResult.Failure)
    }

    @Test
    fun `InMemoryAuthProvider default PIN is 6 digits`() {
        val provider = com.ymr.lanlink.core.data.auth.InMemoryAuthProvider()
    ...
    fun `PIN must be exactly 6 digits`() {
        assertThrows(IllegalArgumentException::class.java) {
            com.ymr.lanlink.core.data.auth.InMemoryAuthProvider("12345")  // Too short
        }
        assertThrows(IllegalArgumentException::class.java) {
            com.ymr.lanlink.core.data.auth.InMemoryAuthProvider("1234567")  // Too long
        }
    }

    @Test
    fun `getCredentials returns correct PIN bytes`() {
        val provider = com.ymr.lanlink.core.data.auth.InMemoryAuthProvider("999888")
    ...
    fun `lockout after 5 failed attempts`() = runBlocking {
        ...
        repeat(5) {
            ...
            assertTrue("Attempt ${it + 1} should fail",
                result is com.ymr.lanlink.core.domain.auth.AuthResult.Failure)
        }

        // 6th attempt should be locked
        val lockedResult = provider.authenticate("TestPeer", "123456".toByteArray())
        assertTrue("6th attempt should be locked",
            lockedResult is com.ymr.lanlink.core.domain.auth.AuthResult.Failure)
    }
}